...
SEARCH_QUERIES = [
    f"工信厅 3D打印 增材制造 项目申报 {YEAR}",
    f"工信厅 医疗器械 高端装备 专项 申报 {YEAR}",
    "工信厅 智能制造 项目申报通知",
    "工信厅 专精特新 小巨人 申报 医疗",
    f"工信厅 新材料 首批次 申报 {YEAR}",
    "工信厅 人工智能 产业 申报指南",
    f"内蒙古 上海 四川 工信厅 项目申报 {YEAR}",
    "安徽 天津 工信厅 技术改造 医疗器械 申报",
]

JSON_SCHEMA = '[{"province":"省份","date":"YYYY-MM-DD","title":"项目名称","department":"发布部门","deadline":"截止日期","funding":"资助额度","summary":"摘要","url":"链接"}]'

SUMMARIZE_PROMPT = f"""从以下搜索结果中筛选与"工信厅项目申报（3D打印/增材制造/智能制造/医疗器械/专精特新/新材料）"相关的内容。
...
def main():
    print(f"[INFO] 工信厅简报 | {PERIOD_START} ~ {TODAY}")
    try:
    ...
    with open("docs/data/gongxinting.json", "w", encoding="utf-8") as f:
        json.dump({"updated": TODAY, "count": len(items), "items": items}, f, ensure_ascii=False, indent=2)
    print(f"[INFO] 已保存 docs/data/gongxinting.json")
