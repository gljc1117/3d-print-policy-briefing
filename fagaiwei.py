#!/usr/bin/env python3
"""
每周自动搜索各省发改委与3D打印/医疗器械/新兴产业相关的项目申报通知。
"""
...
请搜索 **{ONE_MONTH_AGO} 至 {TODAY}** 期间，中国各省发展和改革委员会（发改委）发布的与以下领域相关的项目申报通知、专项资金、产业规划、投资支持政策：
...
1. "发改委 3D打印 增材制造 项目申报 {datetime.now().year}"
2. "发改委 医疗器械 高端装备 申报 {datetime.now().year}"
3. "发改委 战略性新兴产业 生物医药 申报通知"
4. "发改委 中央预算内投资 医疗卫生 申报 {datetime.now().year}"
5. "发改委 数字经济 人工智能 医疗 项目"
6. "发改委 产业创新中心 工程研究中心 申报"
7. "发改委 新材料 先进制造 专项 申报 {datetime.now().year}"
8. "内蒙古 上海 四川 发改委 项目申报 {datetime.now().year}"
9. "安徽 天津 发改委 产业 医疗 投资 申报"

规则：
- 优先包含 {ONE_MONTH_AGO} 之后发布的内容，但高度相关的稍早内容也应包含
- 每条结果必须附带真实的来源链接
- 宁多勿少：只要与发改委项目申报相关，都应包含在输出中
...
def build_html(items: list[dict]) -> str:
    ...
    if not items:
        ...
        <div style="padding:20px;background:#fef9e7;border-left:4px solid #f39c12;margin:16px 0;border-radius:4px;">
            <p style="margin:0;color:#7d6608;">本周期内（近一个月）暂未检索到发改委相关项目申报通知。将持续监测。</p>
        </div>"""
    else:
        cards = []
        for item in items:
...
<div style="background:linear-gradient(135deg,#922b21,{ACCENT});padding:24px 32px;border-radius:8px 8px 0 0;">
  <h1 style="color:#fff;margin:0;font-size:22px;">发改委·项目申报通知周报</h1>
  <p style="color:#f5b7b1;margin:8px 0 0;font-size:14px;">内蒙古子殷科技有限公司 | {TODAY}</p>
</div>
...
def send_email(html: str):
    subject = f"【子殷科技·发改委简报】项目申报通知周报（{TODAY}）"
...
def main():
    print(f"[INFO] 发改委简报 | {ONE_MONTH_AGO} ~ {TODAY}")
    try:
