...
SEARCH_QUERIES = [
    f"发改委 3D打印 增材制造 项目申报 {YEAR}",
    f"发改委 医疗器械 高端装备 申报 {YEAR}",
    "发改委 战略性新兴产业 生物医药 申报通知",
    f"发改委 中央预算内投资 医疗卫生 申报 {YEAR}",
    "发改委 数字经济 人工智能 医疗 项目",
    "发改委 产业创新中心 工程研究中心 申报",
    f"发改委 新材料 先进制造 专项 申报 {YEAR}",
    f"内蒙古 上海 四川 发改委 项目申报 {YEAR}",
    "安徽 天津 发改委 产业 医疗 投资 申报",
]

JSON_SCHEMA = '[{"province":"省份","date":"YYYY-MM-DD","title":"项目名称","department":"发布部门","deadline":"截止日期","funding":"资助额度","summary":"摘要","url":"链接"}]'

SUMMARIZE_PROMPT = f"""从以下搜索结果中筛选与"发改委项目申报（3D打印/医疗器械/战略新兴产业/生物医药/数字经济/新材料）"相关的内容。
...
def main():
    print(f"[INFO] 发改委简报 | {PERIOD_START} ~ {TODAY}")
    try:
    ...
    with open("docs/data/fagaiwei.json", "w", encoding="utf-8") as f:
        json.dump({"updated": TODAY, "count": len(items), "items": items}, f, ensure_ascii=False, indent=2)
    print(f"[INFO] 已保存 docs/data/fagaiwei.json")
