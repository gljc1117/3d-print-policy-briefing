...
SEARCH_QUERIES = [
    f"教育厅 医工交叉 项目申报 {YEAR}",
    f"教育厅 产学研 医疗器械 申报通知 {YEAR}",
    "教育厅 协同创新中心 生物医学 申报",
    "教育厅 重点实验室 3D打印 增材制造 申报",
    "教育厅 高校科研 医学影像 人工智能 项目",
    f"教育厅 校企合作 医疗 申报 {YEAR}",
    f"内蒙古 上海 天津 教育厅 科研项目 医学 申报 {YEAR}",
    "安徽 四川 教育厅 转化医学 生物医学工程 申报",
]

JSON_SCHEMA = '[{"province":"省份","date":"YYYY-MM-DD","title":"项目名称","department":"发布部门","deadline":"截止日期","funding":"资助额度","summary":"摘要","url":"链接"}]'

SUMMARIZE_PROMPT = f"""从以下搜索结果中筛选与"教育厅项目申报（医工交叉/产学研/校企合作/3D打印/生物医学工程/医学影像AI）"相关的内容。
...
def main():
    print(f"[INFO] 教育厅简报 | {PERIOD_START} ~ {TODAY}")
    try:
    ...
    with open("docs/data/jiaoyuting.json", "w", encoding="utf-8") as f:
        json.dump({"updated": TODAY, "count": len(items), "items": items}, f, ensure_ascii=False, indent=2)
    print(f"[INFO] 已保存 docs/data/jiaoyuting.json")
